...
class GoogleFontApiFontInfoRecord {
  ...
  GoogleFontApiFontInfoRecord({required this.family,
...
Future<void> main(List<String> args) async {
  final parser = ArgParser()
    ...
    ..addOption('googlefontslist',
        abbr: 'g',
        defaultsTo: 'missing',
        help: 'This argument should be followed by the path to the local file containing the list of fonts included within the current googlefonts package (the version specified within pubspec.yaml).  Generate this list using by running display_googlefonts_fontlist.dart within the samples directory and saving the font list to a local file within this directory.',
    )
    ..addOption('inputjsonfile',
        abbr: 'i',
        defaultsTo: 'missing',
        help: 'This argument should be followed by the path to the local file containing the google fonts api output json.',
    )
    ..addOption('apikey',
        abbr: 'a',
        defaultsTo: 'missing',
        help: 'This argument should be followed by your SECRET Google fonts api key.',
    );
  ...
  if(!suppliedFontList) {
    ...
    try {
      ...
      print('Read ${googleFontsPackageFontList.length} fonts from $googleFontsListFile');
    } catch (e) {
  ...
  if (localJson) {
    ...
    print('Querying Google Fonts API directly using Google Fonts APIKey $googleFontAPIKey');
  ...
  /*  EXAMPLE head of API Json:
{
  ...
  print('Items list in google fonts api json includes ${fontItems.length} fonts');
  ...
  for(final item in fontItems) {
    ...
    bool includeFont = googleFontsPackageFontList.isEmpty || googleFontsPackageFontList.contains(family);
    ...
    if(includeFont && family.isNotEmpty) {
      ...
      for(final variant in variants) {
        if(!variantMap.containsKey(variant)) {
          ...
          print('ERROR: UNKNOWN and UNMAPPED variants were encountered:  $variant in font $family');
        } else {
  ...
  if(unknownUnMappedVariants.isNotEmpty) {
    ...
    print('Please update the `variantMap` constant map within update_constants.dart');
    print('and the `fontWeightValues` array within lib\\src\\constants\\fontweight_map.dart ');
    exit(1);
  }
...
// `generator/update_constants.dart`.

''');
  constantsContent.writeln('const prefsRecentsKey = "font_picker_recents";');
  ...
  for(final fontfamily in sortedFontMapFonts) {
    ...
    int lineLen = 47+fontfamily.length+fontinfo.category.length+
                      variantListStr.length+subsetsListStr.length;
    if(lineLen<80) {
      constantsContent.writeln('\t"$fontfamily": {"category": "${fontinfo.category}", "variants": "$variantListStr", "subsets": "$subsetsListStr",},');
    } else {
      constantsContent.writeln('\t"$fontfamily": {');
...
void printUsage(ArgParser parser) {
  print('''Usage: update_constants.dart [[--googlefontslist | -g] filenamecontaininglist.txt] [[--inputjsonfile | -i] api_output.json] | [[--apikey | -a] YOUR_SECRET_GOOGLE_FONT_API_KEY]
